...
- [x] Phase 7 — web dashboard published as an Artifact, renamed "Premier League Predictor": https://claude.ai/code/artifact/9ad3c12a-43af-4c19-81fe-c091e8aad30d (`export_dashboard.py` -> `build_dashboard.py` -> republish `dashboard.html`)
  - Polish pass: week-over-week delta indicators, recent-form strip, a plain-language "how this works" intro, Attack vs Defense scatter chart, Elo ranked bar chart, an interactive scoreline probability heatmap, an explicit dark mode toggle, team news as a proper table, and a Sources footer
- [x] Phase 10 — fixture congestion (`src/fixture_congestion.py`, `data/other_competitions.csv`): a small manually-maintained log of non-PL fixture dates per team, used to apply a short-rest penalty (<4 days since last match, any competition). Key insight: rest-days only needs to know *when* a team last played, not who they played, so it works even for European ties against clubs we have zero data on. Wired into every live pipeline entry point via `team_news.expected_goals_for_fixture`
- [x] Phase 8 — historical backtest of the full simulator (`src/backtest_season.py`), rewinding to GW5 of a completed season with a leave-one-out promoted-team prior to avoid leakage. On 2024/25: top-4/relegation Brier scores ~5x better than naive baseline; title prediction confidently backed Man City/Arsenal over eventual champion Liverpool -- a real, permanent limit of results-only models (can't foresee injury collapses), not a bug
  ...
  - Traced the title favorite week-by-week (`track_title_race`) to explain the mid-season noise: in 2024/25 and 2025/26 the model backs the wrong team briefly early (GW3-10ish) then corrects fast and stays right; in 2023/24 it stays genuinely volatile through GW36 because that title race really was a historic three-way photo finish -- not a model flaw, honest uncertainty about a race that was actually still open. See `data/processed/title_race_trace.csv` / `.png`
- [x] Phase 9 — ML layer experiment (`src/ml_features.py`, `src/ml_train.py`): 14 leakage-safe features (Elo, rolling form, rest days, promoted flag), XGBoost with a Poisson objective, time-based train/test split (train before 2025/26, test on it). Result: ML combined goals MAE 0.933 vs static-Poisson-snapshot baseline's 0.927 -- essentially tied, not a clear win. With only 1,140 training matches, gradient boosting can't out-learn a simple model with the right built-in structure. Decision: keep the statistical model live, accept this as a legitimate finding rather than chase further
